...
public class BirthWatch {
    ...
    public static string Run(string procName, int runSeconds, string logFile) {
        ...
        try {
            ...
            byte[] blobPat = Encoding.Unicode.GetBytes("3.9.11.17");
            byte[] intPat = new byte[] { 0x63, 0x09, 0x09, 0x64 }; // 0x64090963 (4.9.9.99)
            byte[] intPatOld = new byte[] { 0x51, 0x05, 0x09, 0x63 };
...
$result = [BirthWatch]::Run($ProcName, $RunSeconds, $LogFile)
