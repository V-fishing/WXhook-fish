# patch_v47b.py - adapt to global C_coHook (no 'static')
...
old2 = '''void __fastcall C_coHook(void* dummy, u64 retaddr) {
...
new2 = '''void __fastcall C_coHook(void* dummy, u64 optsP5) {
    (void)dummy;
    if (g_qAH == g_qAT) return;                 // 热路径快速退出
    if (!g_base || !g_mgr2) return;             // mgr 未武装
    if (g_tid2 && GetCurrentThreadId() != g_tid2) return;  // 仅 UP1 线程 (上下文链完备)
